...
object SparkContextCache {
  ...
  private def createCleanup(sc: SparkContext): () => Unit = {
    ...
    closers += sc.callMethod("env").orElse(sc.getField("env")).flatMap(env =>
      env.getField("rpcEnv").flatMap(rpcEnv =>
        rpcEnv.callMethod("fileServer").flatMap(_ match {
          case s if "org.apache.spark.rpc.netty.HttpBasedFileServer" == s.getClass.getName =>
            s.getField("httpFileServer")
          case s if "org.apache.spark.HttpFileServer" == s.getClass.getName => Some(s)
          case _ => None
        })
        .flatMap(fs =>
  ...
  /**
    ...
    */
  private implicit class ObjectRelectionFunctions(obj: Any) {
