...
public class MockLogReader implements LogReader {
  ...
  private void generateWorkflowRunLogs(LoggingContext loggingContext) {
    ...
    if (loggingContext instanceof WorkflowProgramLoggingContext) {
      ...
      systemTag = loggingContext.getSystemTagsMap().get(MapReduceLoggingContext.TAG_MAP_REDUCE_JOB_ID);
      if (systemTag == null) {
        systemTag = loggingContext.getSystemTagsMap().get(SparkLoggingContext.TAG_SPARK_JOB_ID);
      }
